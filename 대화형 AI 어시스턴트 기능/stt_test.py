...
credentials = service_account.Credentials.from_service_account_file('(real api key)')
...
def main():
    ...
    while True:
        with MicrophoneStream(RATE, CHUNK) as stream:
            ...
            # GPT API로 응답 생성
            df = pd.read_csv("D:\Speech-to-Text\menu_sample.csv")
            pandas_ai = PandasAI(llm, verbose=True)
            response = pandas_ai.run(df, prompt=transcript)
